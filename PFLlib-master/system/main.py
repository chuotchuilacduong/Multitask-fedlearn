...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            ...
            if "MNIST" in args.dataset:
                # Giả sử HQCNN_Ang_noQP là mô hình HQCNN đã được định nghĩa trong models.py
                weight_shapes = {
                    "weights_0": 3,
                    "weights_1": 3,
                    "weights_2": 1,
                ...
                # Thêm các điều kiện cho các dataset khác nếu cần
                weight_shapes = {
                    "weights_0": 3,
                    "weights_1": 3,
                    "weights_2": 1,
                ...
                args.model = HQCNN_Ang_noQP(in_features=60, num_classes=args.num_classes, weight_shapes=weight_shapes).to(args.device)
        ...
        if args.algorithm == "FedAvg":
            args.head = copy.deepcopy(args.model.classical_layer_2)  # Lấy ra đầu ra từ lớp classical
            args.model.classical_layer_2 = nn.Identity()  # Thay thế lớp classical output layer bằng nn.Identity()
            server = FedAvg(args, i)
